...
export async function GET(request) {
  ...
  try {
    await ensureSchema();
    const q = database();
    const rows = await q`SELECT id, workspace_id FROM tl_connections WHERE provider = 'tiktok_shop' AND metadata->>'state' = ${state} LIMIT 1`;
    ...
    if (!response.ok || payload?.code !== 0 || !token?.access_token) return redirect(request, `token_error&reason=${encodeURIComponent(String(payload?.message || payload?.code || response.status).slice(0, 120))}`);

    const metadata = { sellerName: token.seller_name || null, sellerBaseRegion: token.seller_base_region || null, grantedPermissions: token.granted_permissions || [], connectedAt: new Date().toISOString() };
    ...
    return redirect(request, `error&reason=${encodeURIComponent(String(error?.message || "unknown").slice(0, 120))}`);
  }
}
