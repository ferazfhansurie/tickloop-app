...
export async function POST(request) {
  try {
    ...
    const connections = await q`SELECT id,workspace_id FROM tl_connections WHERE provider='whatsapp' AND metadata->>'workerTokenHash'=${tokenHash} LIMIT 1`;
    ...
    if (body.type === "status") {
      ...
      await q`UPDATE tl_connections SET status=${status},external_id=${body.phone || null},updated_at=now() WHERE id=${connection.id}`;
