...
export default function Page() {
  ...
  async function logout() { await fetch("/api/auth/logout", { method: "POST" }); setUser(null); setWorkspace(null); setConversations([]); }
  ...
  return <main className="emptyApp"><aside className="simpleRail"><BrandMark /><div className="railGrow"/><button className="accountButton" onClick={logout} title="Sign out">{user.name?.[0]?.toUpperCase()}</button></aside><header className="mobileHeader"><div><BrandMark className="mobileMark" /><b>TickLoop</b></div><button onClick={logout}>Sign out</button></header><aside className="inboxColumn"><header><h1>Inbox</h1><p>{conversations.length ? `${conversations.length} conversation${conversations.length === 1 ? "" : "s"}` : "All conversations"}</p></header><div className="conversationList">{conversations.length ? conversations.map(item => <button key={item.id} className={`conversationItem ${active?.id === item.id ? "selected" : ""}`} onClick={() => setActiveConversation(item.id)}><span className="conversationAvatar">{(item.customer_name || "C")[0].toUpperCase()}</span><span><b>{item.customer_name || "Customer"}</b><small>{item.provider === "whatsapp" ? "WhatsApp" : "TikTok Shop"} · {item.last_message || "New order"}</small></span></button>) : <div className="emptyList"><strong>No conversations yet</strong><span>New WhatsApp messages and TikTok order events appear here automatically.</span></div>}</div></aside><section className="inboxEmpty">{active ? <div className="conversationPreview"><p className="eyebrow">{active.provider === "whatsapp" ? "WHATSAPP MESSAGE" : "TIKTOK SHOP ORDER"}</p><h2>{active.customer_name || "Customer"}</h2><p className="conversationPhone">{active.customer_phone || "Customer contact protected"}</p><div className="messageBubble">{active.last_message || "A new customer event arrived."}</div><p className="syncHint">New inbound WhatsApp messages sync here automatically, usually within a few seconds.</p></div> : <div className="readyCard"><div className="readyMark">✓</div><p className="eyebrow">YOUR INBOX IS READY</p><h2>Start by connecting your channels.</h2><p>When TikTok Shop orders and WhatsApp messages arrive, they will appear here as real customer conversations.</p><div className="setupLine"><span className={tiktok?.status === "connected" ? "complete" : ""}>{tiktok?.status === "connected" ? "✓" : "1"}</span><b>Connect TikTok Shop</b><small>{tiktok?.status === "connected" ? "Connected" : "Order context"}</small></div><div className="setupLine"><span className={whatsapp?.status === "connected" ? "complete" : ""}>{whatsapp?.status === "connected" ? "✓" : "2"}</span><b>Connect WhatsApp</b><small>{whatsapp?.status === "connected" ? "Connected" : "Customer messages"}</small></div>{tiktok?.status === "connected" && <button className="testOrderButton" onClick={createTestOrder} disabled={creatingTestOrder}>{creatingTestOrder ? "Creating test order…" : "Create a test TikTok order"}</button>}<p className="testNote">Tests stay inside TickLoop. No WhatsApp message is sent.</p><div className="mobileConnectActions"><button className="mobileConnect" onClick={() => connect("tiktok_shop")}>{tiktok?.status === "connected" ? "TikTok Shop connected" : "Connect TikTok Shop"}</button><button className="mobileConnect secondary" onClick={() => connect("whatsapp")}>{whatsapp?.status === "connected" ? "WhatsApp connected" : whatsapp?.status === "pending" ? "Continue WhatsApp setup" : "Connect WhatsApp"}</button></div></div>}</section><aside className="setupPanel"><h2>Workspace setup</h2><p className="setupIntro">Connect your channels before your inbox starts receiving customers.</p><section className="connectionCard"><div><b>TikTok Shop</b><p>{tiktok?.status === "connected" ? "Your seller account is connected." : "Add order context to every conversation."}</p></div><button className="connectButton" onClick={() => connect("tiktok_shop")}>{tiktok?.status === "connected" ? "Connected" : "Connect TikTok Shop"}</button></section><section className="connectionCard"><div><b>WhatsApp</b><p>{whatsapp?.status === "connected" ? "Your WhatsApp number is connected." : "Bring new customer messages into TickLoop."}</p></div><button className="connectButton" onClick={() => connect("whatsapp")}>{whatsapp?.status === "connected" ? "Connected" : whatsapp?.status === "pending" ? "Continue WhatsApp setup" : "Connect WhatsApp"}</button></section>{tiktok?.status === "connected" && <section className="connectionCard automationCard"><div><b>Post-purchase testing</b><p>Create a dummy paid order and confirm that order context reaches the inbox.</p></div><button className="connectButton" onClick={createTestOrder} disabled={creatingTestOrder}>{creatingTestOrder ? "Creating…" : "Create test order"}</button></section>}<div className="workspaceBlock"><small>WORKSPACE</small><b>{workspace?.user?.workspace_name || "Your TickLoop workspace"}</b><span>{user.email}</span>{user.role === "admin" && <a className="adminLink" href="/admin">Open admin dashboard</a>}</div>{setupReady && <p className="allReady">✓ Both channels are connected.</p>}</aside>{notice && <button className="notice" onClick={() => setNotice("")}>{notice}</button>}</main>;
}
