...
export default function Page() {
  ...
  return <main className="emptyApp" data-mobile={mobileView}>
    ...
    <aside className="inboxColumn"><header><h1>Inbox</h1><p>{conversations.length ? `${conversations.length} conversation${conversations.length === 1 ? "" : "s"}` : "All conversations"}</p></header><div className="conversationList">{conversations.length ? conversations.map(item => <button key={item.id} className={`conversationItem ${active?.id === item.id ? "selected" : ""}`} onClick={() => { setActiveConversation(item.id); setMobileView("thread"); }}><ConversationAvatar conversation={item} /><span><b>{item.customer_name || "Customer"}</b><small>{item.provider === "whatsapp" ? "WhatsApp" : "TikTok Shop"} · {item.last_message || "New order"}</small></span></button>) : <div className="emptyList"><strong>No conversations yet</strong><span>New WhatsApp messages and TikTok order events appear here automatically.</span></div>}</div></aside>
