...
export default function Page() {
  ...
  return <main className="emptyApp" data-mobile={mobileView} data-view={view} data-setup={setupOpen ? "open" : "closed"}>
    ...
    <header className="mobileHeader">{mobileView === "list" ? <div className="mobileBrand"><BrandMark className="mobileMark" /><b>TickLoop</b></div> : <button className="mobileBack" onClick={() => setMobileView("list")}>‹ Inbox</button>}<div className="mobileActions"><button className={`mobileTab${mobileView === "setup" ? " on" : ""}`} onClick={() => setMobileView(mobileView === "setup" ? "list" : "setup")}>Setup</button><button className="mobileTab" onClick={logout}>Sign out</button></div></header>
    ...
    <nav className="tabBar">
      ...
      <button className={mobileView === "setup" ? "on" : ""} onClick={() => setMobileView(mobileView === "setup" ? "thread" : "setup")}>
        <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="3.2"/><path d="M12 2.6v3M12 18.4v3M2.6 12h3M18.4 12h3M5.4 5.4l2.1 2.1M16.5 16.5l2.1 2.1M18.6 5.4l-2.1 2.1M7.5 16.5l-2.1 2.1"/></svg>
        Setup
      </button>
    </nav>
    <aside className="setupPanel"><div className="setupHead"><h2>Workspace setup</h2><button className="setupClose" onClick={() => setSetupOpen(false)} title="Hide panel">✕</button></div><p className="setupIntro">Connect your channels before your inbox starts receiving customers.</p><section className="connectionCard"><div><b>TikTok Shop</b><p>{tiktok?.status === "connected" ? "Your seller account is connected." : "Add order context to every conversation."}</p></div><button className={`connectButton${tiktok?.status === "connected" ? " isConnected" : ""}`} onClick={() => { if (tiktok?.status === "connected") { if (!window.confirm("TikTok Shop is already connected. Reconnect it?")) return; } connect("tiktok_shop"); }}>{tiktok?.status === "connected" ? "✓ Connected" : tiktok?.status === "authorizing" ? "Continue connecting" : "Connect TikTok Shop"}</button></section><section className="connectionCard"><div><b>WhatsApp</b><p>{whatsapp?.status === "connected" ? "Your WhatsApp number is connected." : "Bring new customer messages into TickLoop."}</p></div><button className={`connectButton${whatsapp?.status === "connected" ? " isConnected" : ""}`} onClick={() => { if (whatsapp?.status === "connected") { if (!window.confirm("WhatsApp is already connected. Re-pair it?")) return; } connect("whatsapp"); }}>{whatsapp?.status === "connected" ? "✓ Connected" : whatsapp?.status === "pairing" ? "Continue pairing" : "Connect WhatsApp"}</button></section><section className="connectionCard automationCard"><div><b>Post-purchase testing</b><p>Create a dummy paid order and confirm that order context reaches the inbox.</p></div><button className="connectButton" onClick={createTestOrder} disabled={creatingTestOrder}>{creatingTestOrder ? "Creating…" : "Create test order"}</button></section><section className="connectionCard testSendCard"><b>Send a direct test</b><p>Use a number you control or have permission to message.</p><form onSubmit={sendTestMessage}><input required value={testSend.phone} onChange={event => setTestSend({ ...testSend, phone: event.target.value })} placeholder="+60 12 345 6789" /><textarea required value={testSend.message} onChange={event => setTestSend({ ...testSend, message: event.target.value })} /><label className="consentCheck"><input type="checkbox" checked={testSend.confirmed} onChange={event => setTestSend({ ...testSend, confirmed: event.target.checked })} /> I have permission to message this number.</label><button className="connectButton" disabled={sendingTest}>{sendingTest ? "Sending…" : "Send test message"}</button></form></section><div className="workspaceBlock"><small>WORKSPACE</small><b>{workspace?.user?.workspace_name || "Your TickLoop workspace"}</b><span>{user.email}</span>{user.role === "admin" && <a className="adminLink" href="/admin">Open admin dashboard</a>}</div>{setupReady && <p className="allReady">✓ Both channels are connected.</p>}</aside>
