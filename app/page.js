...
const KIND_ICON = { video: "▶", audio: "♪", image: "🖼", sticker: "🖼", document: "📄" };

// The sync writes "[Image]" / "[Video]" etc. as the body when an attachment has
// no caption, so the row is never empty. Once the attachment card is on screen
// that placeholder is just noise — show real captions only.
const MEDIA_PLACEHOLDER = /^\[(image|video|audio|document|sticker)\]$/i;
function captionOf(message) {
  const body = (message.body || "").trim();
  if (!body) return "";
  if (message.media_kind && MEDIA_PLACEHOLDER.test(body)) return "";
  return message.body;
}

// Attachments behave like WhatsApp: nothing is fetched until you ask for it.
// Tap the card to download (the paired laptop fetches and uploads the bytes),
// and once they land the preview renders inline from then on.
function Attachment({ message, onDownload, requesting }) {
  const ready = !!message.media_id;
  const src = ready ? `/api/media/${message.media_id}` : null;
  const kind = message.media_kind;
  const unavailable = message.media_state === "unavailable";
  const pending = requesting || message.media_state === "requested";

  if (ready) {
    if (kind === "image" || kind === "sticker") {
      return <a href={src} target="_blank" rel="noreferrer" className="mediaImageLink" title="Open full size"><img className="mediaImage" src={src} alt={message.media_name || "Photo"} /></a>;
    }
    if (kind === "video") return <video className="mediaVideo" src={src} controls preload="metadata" />;
    if (kind === "audio") return <audio className="mediaAudio" src={src} controls preload="metadata" />;
    return (
      <a className="mediaFile" href={src} target="_blank" rel="noreferrer" download={message.media_name || undefined}>
        <span className="mediaIcon">📄</span>
        <span className="mediaMeta"><b>{message.media_name || "Document"}</b><small>{fileSize(message.media_size) || "Open"}</small></span>
      </a>
    );
  }
  ...
  return (
    <button
      type="button"
      className={`mediaFile mediaDownload${unavailable ? " gone" : ""}`}
      disabled={pending || unavailable}
      onClick={() => onDownload(message)}
    >
      <span className="mediaIcon">{pending ? "…" : unavailable ? "✕" : "⤓"}</span>
      <span className="mediaMeta">
        <b>{message.media_name || `${kind ? kind[0].toUpperCase() + kind.slice(1) : "File"}`}</b>
        <small>
          {unavailable ? "No longer on WhatsApp" : pending ? "Downloading…" : `${KIND_ICON[kind] || "📄"} Tap to download`}
        </small>
      </span>
    </button>
  );
...
export default function Page() {
  ...
  const [requestedMedia, setRequestedMedia] = useState(() => new Set());
  // Mobile is one pane at a time: the columns that sit side by side on desktop
  // become list -> thread -> setup views driven by this.
  const [mobileView, setMobileView] = useState("list");
  // Finance is the daily view; the inbox is a task you navigate to.
  const [view, setView] = useState("finance");
  const [profileOpen, setProfileOpen] = useState(false);
  const { mode: themeMode, setMode: setThemeMode } = useTheme();
  const fileInputRef = useRef(null);

  // Ask the laptop to fetch this attachment. The thread poll picks up the bytes
  // a few seconds later and swaps the card for a real preview.
  async function downloadMedia(message) {
    setRequestedMedia(current => new Set(current).add(message.id));
    try {
      ...
      if (data.state === "unavailable") setNotice(data.error || "WhatsApp no longer has this attachment.");
      window.setTimeout(() => loadThread(effectiveConversationId, { quiet: true }), 2500);
    } catch {
      setRequestedMedia(current => { const next = new Set(current); next.delete(message.id); return next; });
    }
  }
  ...
  return <main className="emptyApp" data-mobile={mobileView} data-view={view}>
    ...
    <nav className="tabBar">
      ...
      <button className={view === "inbox" && mobileView !== "setup" ? "on" : ""} onClick={() => { setView("inbox"); setMobileView("list"); }}>
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5h16v11a2 2 0 0 1-2 2H8l-4 3z"/></svg>
        Inbox{conversations.length ? ` (${conversations.length})` : ""}
      </button>
      <button className={profileOpen ? "on" : ""} onClick={() => setProfileOpen(true)}>
        <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8.5" r="3.6"/><path d="M4.8 20c.8-3.6 3.7-5.6 7.2-5.6s6.4 2 7.2 5.6"/></svg>
        Profile
      </button>
    </nav>
    {profileOpen && (
      <>
        <button className="sheetScrim" onClick={() => setProfileOpen(false)} aria-label="Close profile" />
        <section className="profileSheet" role="dialog" aria-label="Profile">
          <header>
            <span className="profileAvatar">{user.name?.[0]?.toUpperCase()}</span>
            <div><b>{user.name}</b><small>{user.email}</small></div>
            <button className="profileClose" onClick={() => setProfileOpen(false)} aria-label="Close">✕</button>
          </header>
          <div className="profileBlock">
            <small>WORKSPACE</small>
            <b>{workspace?.user?.workspace_name || "Your TickLoop workspace"}</b>
          </div>
          <div className="profileBlock">
            <small>APPEARANCE</small>
            <div className="themeChips">
              {["system", "light", "dark"].map(option => (
                <button key={option} className={themeMode === option ? "on" : ""} onClick={() => setThemeMode(option)}>{THEME_LABEL[option]}</button>
              ))}
            </div>
          </div>
          <div className="profileBlock">
            <small>CHANNELS</small>
            <div className="profileRow"><span>TikTok Shop</span><b className={tiktok?.status === "connected" ? "ok" : "no"}>{tiktok?.status === "connected" ? "Connected" : "Not connected"}</b></div>
            <div className="profileRow"><span>WhatsApp</span><b className={whatsapp?.status === "connected" ? "ok" : "no"}>{whatsapp?.status === "connected" ? "Connected" : "Not connected"}</b></div>
            <div className="profileRow"><span>Business Center</span>{business?.status === "connected" ? <b className="ok">Connected</b> : <a className="finLink" href="/api/business/connect">Connect</a>}</div>
          </div>
          <div className="profileActions">
            {user.role === "admin" && <a className="connectButton" href="/admin">Admin dashboard</a>}
            <button className="connectButton" onClick={logout}>Sign out</button>
          </div>
        </section>
      </>
    )}
    ...
    {forwarding && <div className="forwardOverlay" onClick={() => setForwarding(null)}><div className="forwardCard" onClick={event => event.stopPropagation()}><header><b>Forward to…</b><button type="button" onClick={() => setForwarding(null)}>✕</button></header><p className="forwardPreview">{forwarding.media_kind ? `📎 ${forwarding.media_name || forwarding.media_kind} ` : ""}{captionOf(forwarding).slice(0, 120)}</p><div className="forwardList">{conversations.filter(item => item.id !== effectiveConversationId && item.provider === "whatsapp").map(item => <button key={item.id} type="button" disabled={sending} onClick={() => forwardTo(item.id)}><ConversationAvatar conversation={item} /><span>{item.customer_name || item.customer_phone || "Customer"}</span></button>)}</div></div></div>}
