...
export default function Page() {
  ...
  return <main className="emptyApp">
    ...
    {whatsapp?.status === "connected" && <div className={`syncStatus ${sync.syncStatus || "waiting"}`}><span>{sync.syncStatus === "complete" ? "✓" : "↻"}</span><div><b>{sync.syncStatus === "complete" ? "WhatsApp history synced" : sync.syncStatus === "retrying" ? "Retrying WhatsApp history" : sync.syncStatus === "syncing" ? "Syncing WhatsApp history" : "Live WhatsApp sync active"}</b><small>{sync.syncTotal ? `${sync.syncImported || 0} of ${sync.syncTotal} latest chats imported` : sync.lastSyncError || "New messages appear here automatically."}</small></div></div>}
