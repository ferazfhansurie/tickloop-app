import { createCipheriv, createDecipheriv, createHash, pbkdf2, randomBytes, timingSafeEqual } from "crypto";
...
const adminEmail = () => (process.env.SUPER_ADMIN_EMAIL || "admin@tickloop.com").trim().toLowerCase();
...
export async function registerUser({ name, email, password }) {
  await ensureSchema(); const q = database(); const normalized = email.trim().toLowerCase(); const displayName = name.trim(); const workspaceName = `${displayName} workspace`; const salt = randomBytes(16).toString("hex"); const hash = await passwordHash(password, salt); const userId = id("usr"); const workspaceId = id("ws"); const role = normalized === adminEmail() ? "admin" : "member";
  try { await q`INSERT INTO tl_users (id,email,name,password_salt,password_hash,role) VALUES (${userId},${normalized},${displayName},${salt},${hash},${role})`; await q`INSERT INTO tl_workspaces (id,owner_id,name) VALUES (${workspaceId},${userId},${workspaceName})`; }
