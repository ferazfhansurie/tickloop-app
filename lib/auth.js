...
export async function registerUser({ name, email, password }) {
  await ensureSchema(); const q = database(); const normalized = email.trim().toLowerCase(); const salt = randomBytes(16).toString("hex"); const hash = await passwordHash(password, salt); const userId = id("usr"); const workspaceId = id("ws");
  try { await q`INSERT INTO tl_users (id,email,name,password_salt,password_hash) VALUES (${userId},${normalized},${name.trim()},${salt},${hash})`; await q`INSERT INTO tl_workspaces (id,owner_id,name) VALUES (${workspaceId},${userId},${name.trim()} + ' workspace')`; }
  ...
  return { id: userId, name: name.trim(), email: normalized, workspaceId };
}
