import { callShop, sleep } from "./tiktok";
...
export const LINE_LABELS = {
  ...
  vn_fix_infrastructure_fee: "Infrastructure fee",
...
/**
 * Keys that RESTATE another line rather than adding money of their own. Summing
 * them double-counts: on a live shop, `platform_shipping_fee_discount_amount`
 * repeated `shipping_fee_discount_amount` exactly, turning a -31.80 shipping
 * cost into +1770.40, and `affiliate_commission_amount_before_pit` repeated the
 * post-PIT commission. Excluding these makes each group's subtotal equal the
 * top-level amount TikTok reports for it.
 */
const RESTATEMENT_KEYS = new Set(["affiliate_commission_amount_before_pit"]);

/**
 * Flattens one breakdown object into signed lines, dropping zeroes.
 *
 * `supplementary_component` is deliberately NOT recursed into: it is an
 * informational re-cut of the parent fields, not extra money.
 */
function flatten(source, group, into) {
  if (!source) return;
  for (const [key, value] of Object.entries(source)) {
    if (value && typeof value === "object") continue;
    if (RESTATEMENT_KEYS.has(key)) continue;
 ...
 */
export async function fetchSettlements({ accessToken, shopCipher, statementTimeGe, statementTimeLt, maxTransactions = 5000, skipStatementIds }) {
  const rows = [];
  const statements = [];
  ...
  outer: do {
    ...
    for (const statement of page.data?.statements || []) {
      ...
      // Statements are immutable once paid out, so anything already stored can be
      // skipped. Without this, a throttled sync re-walks the same statements every
      // attempt and never reaches the ones it has not read yet.
      if (skipStatementIds?.has(statement.id)) { skipped += 1; continue; }
      await sleep(120); // pace the burst rather than rely on retries alone
  ...
  return { rows, statements, sampleTransaction, truncated, error, skipped };
}
