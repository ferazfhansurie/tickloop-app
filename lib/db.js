...
export async function ensureSchema() {
  ...
  ready = (async () => {
    ...
    // Keep WhatsApp's own message key so an attachment can be fetched on demand
    // later (the bytes only live on WhatsApp's servers + the paired laptop).
    // media_state: null = never asked, 'requested' = user tapped download,
    // 'unavailable' = WhatsApp no longer has it.
    await q`ALTER TABLE tl_messages ADD COLUMN IF NOT EXISTS wa_key jsonb`;
    await q`ALTER TABLE tl_messages ADD COLUMN IF NOT EXISTS media_state text`;
    await q`CREATE INDEX IF NOT EXISTS tl_messages_media_queue_idx ON tl_messages (media_state) WHERE media_state='requested'`;
