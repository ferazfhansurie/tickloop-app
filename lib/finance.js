...
export async function financeSummary(workspaceId, fromIso, toIso, basis = "order") {
  ...
  const adsGmvPay = adsGmvPayIsOverride
    ...
    : adSpend.gmvPay > 0 ? adSpend.gmvPay : Math.abs(totals.adFee);
  const adsGmvPayIsSynced = !adsGmvPayIsOverride && adSpend.gmvPay > 0;
  ...
  const operatingProfit = duitMasuk - kosProdukSettled - adsCard - wht - otherCost;
