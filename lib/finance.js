...
export async function creatorLeaderboard(workspaceId, orderIds, costBySku) {
  ...
  const q = database();
  const rows = await q`SELECT a.creator, a.content_type, a.commission, o.order_id, o.total_amount, o.items,
    ...
    LEFT JOIN (SELECT order_id, sum(settlement_amount) AS settled FROM tl_settlements WHERE workspace_id=${workspaceId} GROUP BY order_id) s
  ...
  for (const row of rows) {
    ...
    entry.commission += Math.abs(Number(row.commission) || 0);
