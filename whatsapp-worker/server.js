...
async function pullOutbox() {
  if (!connectedPhone || outboxBusy) return;
  outboxBusy = true;
  try {
...
function captureIncoming(message) {
  ...
  const messageId = message.id?._serialized; if (!messageId || receivedMessageIds.has(messageId)) return;
  receivedMessageIds.add(messageId); if (receivedMessageIds.size > 500) receivedMessageIds.delete(receivedMessageIds.values().next().value);
...
client.on("message", captureIncoming);
client.on("message_create", captureIncoming);
